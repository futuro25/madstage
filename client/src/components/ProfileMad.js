...
export default function ProfileMad() {
  ...
  const [isDesktopPlatform, setIsDesktopPlatform] = useState(
    window.innerWidth > 768
  );
  ...
  const onChange = async (images) => {
    ...
    await mutate(
      API_URL,
      utils.patchRequest(`${API_URL}/${sessionStorage.userId}`, {
        ...
        <div className="px-4 h-full overflow-auto mt-0">
          ...
          <div className="mt-4">
            <div className="flex items-center justify-between ml-auto">
              <h2 className="inline-block font-extrabold text-gray-800 tracking-tight text-xl">
                Images
              </h2>
              ...
              <div className="flex ml-auto gap-2 h-10 overflow-hidden">
                {isButtonsEnabled ? (
                  <Button
                    className="h-5 px-1 w-16"
                    variant="destructive-outlined"
                    onClick={() => setIsButtonsEnabled(!isButtonsEnabled)}
                  >
                    Cancelar
                  </Button>
                ) : (
            ...
            <div
              className={cn(
                "flex items-center justify-end ml-auto gap-2 h-0 overflow-hidden transition-all duration-200",
                { "h-12": isButtonsEnabled }
              )}
            >
              <input
                className="hidden"
              ...
              {showInstagramImport && (
                <InstagramImport
                  onClose={setShowInstagramImport}
                  onChange={onChange}
              ...
              {ref.current?.value && (
                <Button
                  className="h-5 px-1 w-16"
                  onClick={() =>
                    !loading ? handleUpload() : console.log("disabled")
                  }
                >
            ...
            <div
              className={cn(
                "grid mt-2",
                isMobilePlatform && "grid-cols-3 gap-2",
                isDesktopPlatform && "grid-cols-8 gap-2"
              )}
            >
              {isDesktopPlatform &&
                user.pictures.map((image) => (
                  <div className="grid place-items-center h-30 border p-1 border-gray-200 cursor-pointer">
                    <img
                      src={image.url}
                      className="h-28 transition-transform duration-200 hover:scale-105"
              ...
              {isMobilePlatform &&
                user.pictures.map((image) => (
                  <div className="grid place-items-center h-30 border p-1 border-gray-200 cursor-pointer">
                    <img
                      src={image.url}
                      className="flex items-center justify-center h-20 w-30 object-cover"
                      onClick={() => showImage(image.url)}
                    />
                    <Button
                      variant={"destructive-outlined"}
          ...
          <div className="flex items-center justify-between ml-auto mt-4">
            ...
            <Button
              className="ml-auto h-5 px-1 w-16"
          ...
          <div>
            <div className="grid grid-cols-4 gap-2 mt-2 mb-20">
